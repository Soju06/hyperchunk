...
RADIUS="${HYPERCHUNK_RADIUS:-5}"
RUN_DIR="${HYPERCHUNK_RUN_DIR:-$HERE/work/stagedump-diag-run}"
BG_THREADS="${HYPERCHUNK_BG_THREADS:-1}"
DUMP_DIR="${HYPERCHUNK_DUMP_DIR:-$HERE/work/stagedump-diag-bundle}"
...
java -Xms2G -Xmx4G -Dmax.bg.threads="$BG_THREADS" \
    ...
    -Dhyperchunk.dump.centerX=${HYPERCHUNK_CENTER_X:-0} \
    -Dhyperchunk.dump.centerZ=${HYPERCHUNK_CENTER_Z:-0} \
...
BLK_MIN_X=$(( (${HYPERCHUNK_CENTER_X:-0} - RADIUS) * 16 )); BLK_MIN_Z=$(( (${HYPERCHUNK_CENTER_Z:-0} - RADIUS) * 16 ))
BLK_MAX_X=$(( (${HYPERCHUNK_CENTER_X:-0} + RADIUS) * 16 + 15 )); BLK_MAX_Z=$(( (${HYPERCHUNK_CENTER_Z:-0} + RADIUS) * 16 + 15 ))
echo "forceload add $BLK_MIN_X $BLK_MIN_Z $BLK_MAX_X $BLK_MAX_Z" >&3
...
# every grid chunk decorates exactly once (센터 오프셋 그리드)
CCX="${HYPERCHUNK_CENTER_X:-0}"
CCZ="${HYPERCHUNK_CENTER_Z:-0}"
for CX in $(seq "$((CCX - RADIUS))" "$((CCX + RADIUS))"); do
    for CZ in $(seq "$((CCZ - RADIUS))" "$((CCZ + RADIUS))"); do
...
if [[ "$SEED" =~ ^[0-9]+$ ]] &&
   (( CCX - RADIUS <= 0 && 0 <= CCX + RADIUS && CCZ - RADIUS <= 0 && 0 <= CCZ + RADIUS )); then
