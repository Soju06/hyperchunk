/* scalar-vs-avx2 백엔드 동일성 게이트 — 유닛 판 (P2-4, ADR-004 D4).
 *
 * 같은 그래프·같은 좌표 시퀀스를 스칼라 백엔드 nc 와 AVX2 백엔드 nc 로
 * 이중 실행해 관측면 (fill_slice 슬라이스 값, select_cell_yz 의
 * density_cell) 을 IEEE-754 비트로 대조한다. x4 커널이 지나는 전 op
 * (NOISE 33, BLENDED, YCG, choice 컨트롤, INTERP lerp3, jmin/jmax ±0/NaN
 * 선택 로직) 이 실좌표 분포로 커버된다. 리전 판은
 * scripts/check_isa_equiv.sh (풀 리전 canonical 이중 실행).
 *
 * AVX2 부재 호스트는 SKIP (exit 77, ctest SKIP_RETURN_CODE) — 그 호스트
 * 에서는 두 백엔드가 동일 코드라 게이트가 공허하다.
 *
...
static void check_bits(const char *what, int32_t i0, int32_t i1, double got,
    ...
    if (gb != wb) {
        ...
        if (g_fails <= 40)
            fprintf(stderr,
                    "FAIL %s [%d,%d]: avx2 %.17g (0x%016" PRIx64
...
int main(int argc, char **argv) {
    if (argc != 2) {
        fprintf(stderr, "usage: test_df_x4 <ref_dir>\n");
        return 2;
    }
    hc_arena_init(&g_arena, g_backing, sizeof g_backing);

    /* AVX2 부재 호스트 → SKIP (강제가 스칼라로 강등되면 부재) */
    hc_isa_force(HC_ISA_AVX2);
    if (hc_isa_active() != HC_ISA_AVX2) {
        fprintf(stderr, "SKIP: host lacks AVX2 — backend gate vacuous\n");
        return 77;
    }
...
#undef S
    ...
    for (int ci = 0; ci < 2; ci++) {
        ...
        if (!ncv || hc_nc_init(ncv, &g_arena, &graph, &roots, seed, cx, cz, 63))
            die("hc_nc_init avx2 failed (arena?)", NULL);
        assert(ncv->x4 == 1);
        /* 26.2 핫 콘은 x4 적격이어야 한다 — 화이트리스트 회귀 fail-loud */
    ...
    printf("test_df_x4: %d nodes; %d checks, %d failures\n", graph.n,
           g_checks, g_fails);
