/* 05_surface 스테이지 청크 패리티 (Plan Task 7 게이트).
 *
 * 스테이지 체이닝 증명: 04_noise 를 '우리' 구현으로 생성한 상태에서
 * 출발해 (골든 04 재생이 아니라), surface 를 적용하고
 * golden/stages/seed<seed>/c.<x>.<z>/05_surface.{blocks,heightmaps}.txt
 * 와 라인 단위 0 diff 를 요구한다.
 *
 * 유일한 골든 유래 입력 = 바이옴 (바이옴 생성은 후속 태스크):
 *  - 3x3 청크: golden 03_biomes.biomes.txt (스테이지 덤프 — 04/05 와
 *    바이트 동일함을 서베이로 확인)
 *  - 5x5 외곽 링: golden/rng/surface_seed*.txt 의 quart_biomes 섹션
 *    (경계 청크의 줌이 3x3 밖 쿼트를 읽는다)
 *  두 소스가 겹치는 3x3 구간은 전 쿼트 일치를 강제해 (fail-loud)
 *  quart_biomes 가 청크 저장소와 같음을 증명한다. hc_chunk_t.biomes 도
 *  같은 데이터로 채운다 (03_biomes 스테이지가 남겼을 상태).
 *
 * false-PASS 방어는 test_noise_stage.c 와 동일 (골든 형식 검증, 라인 수
 * 계상, 총 라인 인쇄). */
...
/* --- reference/ 로더 (test_noise_stage.c 와 동일 규약) --- */
...
/* --- 5x5 쿼트 바이옴 그리드 (quart_biomes 섹션 + 03_biomes 대조) --- */

enum { QG_MIN_XZ = -8, QG_NXZ = 20, QG_MIN_Y = -16, QG_NY = 96 };

static uint16_t       g_grid[QG_NY][QG_NXZ][QG_NXZ]; /* intern id */
...
/* 03_biomes 덤프를 파싱해 그리드와 전 쿼트 대조 + chunk->biomes 채움 */
        ...
        *nl = '\0';
        if (p[0] == '#') {
            ...
            for (int qx = 0; qx < 4; qx++) {
                ...
                /* 그리드(quart_biomes)와 대조 — 두 골든 소스의 일치 증명 */
...
/* --- 기후 로드 (reference/biome_climate-26.2.json) --- */
...
/* --- 렌더러 (FORMAT.md — test_noise_stage.c 와 동일, stage 매개변수화,
 * 블록 이름은 공용 hc_block_name) --- */
...
/* --- 라인 단위 대조 (test_noise_stage.c 와 동일) --- */
...
int main(int argc, char **argv) {
    if (argc != 5) {
        fprintf(stderr, "usage: test_surface_stage <ref_dir> "
    ...
    if (g_n_dfs < 19 || g_n_noises < 51)
        die("reference closure incomplete (needs surface noises)", ref_dir);
    ...
    /* 라우터 컴파일 (04_noise 와 동일 경로 — 스테이지 체이닝의 앞 절반) */
    ...
    /* surface 시스템 (룰 트리 컴파일 — 바이옴 intern 포함) */
    hc_surface_t *surf =
    ...
    /* 바이옴 입력: quart_biomes 그리드 + (아래 청크 루프에서) 03_biomes
     * 대조. 그 다음 기후 로드 (모든 intern 이 끝난 뒤가 아니라도 나중
     * intern 은 없다 — 청크 루프의 intern 은 이미 팔레트에 있던 이름뿐). */
    ...
    for (size_t ci = 0; ci < sizeof CHUNKS / sizeof CHUNKS[0]; ci++) {
        ...
        /* 스테이지 체인: 우리 04_noise → (골든 바이옴) → 우리 05_surface */
        ...
        for (int kind = 0; kind < 2; kind++) {
            sb.len = 0;
            if (kind == 0)
                render_blocks(&sb, &chunk, "surface");
            else
                render_heightmaps(&sb, &chunk, "surface");
            sb.buf[sb.len] = '\0';

            char path[1024], what[64];
            snprintf(path, sizeof path, "%s/c.%d.%d/05_surface.%s.txt",
    ...
    printf("test_surface_stage: %d rules, %d conds, %d samplers; 9 chunks "
           "(own 04_noise chained), %d golden lines compared, %d diffs\n",
           surf->n_rules, surf->n_conds, surf->n_samplers, total_lines,
           g_fails);
