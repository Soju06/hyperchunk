...
.github/CONTRIBUTING.md § Commit convention):
    ...
    text       = non-empty; English or Korean both fine
...
def check(subjects):
    ...
    bad = [s for s in subjects if not EXEMPT_RE.match(s) and not SUBJECT_RE.match(s)]
    for s in bad:
        print(f"NONCONFORMING: {s if s else '(empty subject)'}", file=sys.stderr)
    if bad:
        print(
            f"\n{len(bad)} commit subject(s) violate the convention "
            "'type(scope): subject' — see .github/CONTRIBUTING.md § Commit convention.",
            file=sys.stderr,
        )
    return not bad
...
def self_test():
    good = [
        ...
        "brand: v17 배너(ivory/dark)+마크 SVG — README 히어로 장착",
        "bench(micro)+notes(bench): B-4 종결 판정 노트 — 적격성 4항목",
    ...
    ok = True
    for s in good:
        if not (EXEMPT_RE.match(s) or SUBJECT_RE.match(s)):
            print(f"self-test FAIL (should pass): {s}", file=sys.stderr)
            ok = False
    for s in bad:
        if EXEMPT_RE.match(s) or SUBJECT_RE.match(s):
